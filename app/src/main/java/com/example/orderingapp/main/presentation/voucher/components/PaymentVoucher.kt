...
@Composable
fun PaymentVoucher(orderEntry: OrderEntry, drawerState: BottomDrawerState, scope: CoroutineScope) {
    Column(
        modifier = Modifier
            .fillMaxSize()
            .background(Color.White)
    ) {
        VoucherToolbar(drawerState, scope, orderEntry.key)
        VoucherInfo(orderEntry.value)
    }
}