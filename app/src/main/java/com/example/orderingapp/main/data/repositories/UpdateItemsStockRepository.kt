...
class UpdateItemsStockRepository(private val firestore: FirebaseFirestore) :
    ...
    override fun updateItemsStock(items: Map<String, ItemCompose>): Flow<ApiResult<Unit>> = flow {
        val result = safeRequestSuspend {
            items.forEach {
                firestore.collection(FirestoreCollections.items).document(it.key).set(it.value)
            }
        }
        emit(result)
    }
